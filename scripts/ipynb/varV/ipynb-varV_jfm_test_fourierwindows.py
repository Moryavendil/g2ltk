...
for window in windows:
    Z_pw = utility.fourier.psd2d(Z_mm, x_mm, t_s, window=window, zero_pad_factor=zero_pad_factor, shift=shift, winstyle=winstyle)
    W_pw = utility.fourier.psd2d(W_mm, x_mm, t_s, window=window, zero_pad_factor=zero_pad_factor, shift=shift, winstyle=winstyle)
    ...
    dat[window] = {'Z_pw': Z_pw, 'W_pw': W_pw, }
                   # 'Z_ang': Z_ang, 'W_ang': W_ang}
...
for window in windows:
    zpwmax, zpwmin = utility.log_amplitude_range(dat[window]['Z_pw'].max(), range_db=range_db)
    norm_z = LogNorm(vmax=zpwmax, vmin=zpwmin, clip=True)
    
    wpwmax, wpwmin = utility.log_amplitude_range(dat[window]['W_pw'][:, np.argmin((q - q0/2)**2):].max(), range_db=range_db)
    norm_w = LogNorm(vmax=wpwmax, vmin=wpwmin, clip=True)

    dat[window]['zpwmax'] = zpwmax
    dat[window]['norm_z'] = norm_z
    dat[window]['wpwmax'] = wpwmax
    dat[window]['norm_w'] = norm_w
...
fig, axes = plt.subplots(len(windows)//2+len(windows)%2, 2, sharex=True, sharey=True, figsize=utility.figsize('double', ratio=1))
imshow_kw = {'origin':'upper',
             'interpolation':'nearest',
             'aspect':'auto',
             'extent': utility.correct_extent(q, f)}

fig.suptitle(f'4x zero padding, outer product windowing')
...
for i_window, window in enumerate(windows):
    ...
    im_zpw = ax.imshow(dat[window]['Z_pw'], norm=dat[window]['norm_z'], **imshow_kw)
    cbz = plt.colorbar(im_zpw, ax=ax, label=r'$|\hat{z}|^2$ [mm$^2$/Hz/mm$^{-1}$]')
    utility.set_ticks_log_cb(cbz, dat[window]['zpwmax'], range_db=range_db)
...
# utility.save_graphe('outer_nozero')
# utility.save_graphe('outer_4xzero')
# utility.save_graphe('circular_nozero')
# utility.save_graphe('circular_4xzero')
