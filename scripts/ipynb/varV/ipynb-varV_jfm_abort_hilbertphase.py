...
utility.activate_saveplot(False, style='confort')
...
z1_crossline = map_coordinates(ZX+ZT, np.vstack((t_crossline, x_crossline))).astype(float)
zx_crossline = map_coordinates(ZX, np.vstack((t_crossline, x_crossline))).astype(float)
zt_crossline = map_coordinates(ZT, np.vstack((t_crossline, x_crossline))).astype(float)

plt.figure()

plt.plot(z_crossline**2)
# plt.plot(z1_crossline)
plt.plot(w_crossline)
...
plt.plot(z_crossline**2, w_crossline)
# plt.plot(z1_crossline, w_crossline)
# plt.plot(z1_crossline, w_crossline)
# plt.plot(w_crossline)
...
ax.plot(zx_crossline, zt_crossline, w_crossline)


# <codecell>

from g2ltk.fringeswork import phase_wrapped_hilbert2

phase_Z0 = -phase_wrapped_hilbert2(ZX.copy(), flip=True)
phase_Z1 = -phase_wrapped_hilbert2(ZT.copy(), flip=True)
phase_W1 = -phase_wrapped_hilbert2(W_.copy(), flip=True)
phase_Z = -phase_wrapped_hilbert2(Z_.copy(), flip=True)


# <codecell>

fig, axes = plt.subplots(2, 3, figsize=utility.figsize('double'))

kw = {'aspect': 'auto', 'interpolation': 'nearest'}
kw_wr = {'vmin':-np.pi, 'vmax': np.pi,'cmap': 'twilight', **kw}

p1 = -phase_wrapped_hilbert2(Z_.copy()[:, ::-1], flip=True)
p2 = -phase_wrapped_hilbert2(Z_.copy()[:, ::-1], flip=False)
...
ax = axes[0, 1]
ax.imshow(p1, **kw_wr)
ax.contour(p1, levels=[-np.inf, -np.pi/2, 0, np.pi/2, np.inf], colors='cyan')
ax.contour(p1, levels=[-np.inf, -np.pi, 0, np.pi, np.inf], colors='lime')
ax = axes[0, 2]
ax.imshow(p2, **kw_wr)
ax.contour(p2, levels=[-np.inf, -np.pi/2, 0, np.pi/2, np.inf], colors='cyan')

ax = axes[1, 0]
ax.imshow(Z_, **kw)
ax = axes[1, 1]
ax.imshow(unwrap_phase(p1), **kw)
ax = axes[1, 2]
ax.imshow(unwrap_phase(p2), **kw)
...
im = ax.imshow(phase_Z0, **imshow_kw)
cbwa = plt.colorbar(im, ax=ax)
cbwa.ax.set_yticks([-np.pi, -np.pi/2, 0, np.pi/2, np.pi], minor=False)
cbwa.ax.set_yticklabels([r'$-\pi$', r'$-\pi/2$', r'$0$', r'$\pi/2$', r'$\pi$'])
force_aspect_ratio(ax)
...
im = ax.imshow(phase_Z1, **imshow_kw)
cbwa = plt.colorbar(im, ax=ax)
cbwa.ax.set_yticks([-np.pi, -np.pi/2, 0, np.pi/2, np.pi], minor=False)
cbwa.ax.set_yticklabels([r'$-\pi$', r'$-\pi/2$', r'$0$', r'$\pi/2$', r'$\pi$'])
force_aspect_ratio(ax)
...
im = ax.imshow(wrap(phase_W1 - phase_Z1 - phase_Z0), **imshow_kw)
cbwa = plt.colorbar(im, ax=ax)
cbwa.ax.set_yticks([-np.pi, -np.pi/2, 0, np.pi/2, np.pi], minor=False)
cbwa.ax.set_yticklabels([r'$-\pi$', r'$-\pi/2$', r'$0$', r'$\pi/2$', r'$\pi$'])
force_aspect_ratio(ax)
...
# plt.figure()
# 
# plt.hist(delta.flatten(), bins = np.linspace(-np.pi, np.pi, 501))
# 
...
# plt.plot(wrap(phase_Z1+phase_Z0).flatten(), phase_W1.flatten(), lw=.5, alpha=.2)
# plt.plot(unwrap_phase((phase_Z1+phase_Z0)).flatten(), unwrap_phase(phase_W1).flatten() +2*np.pi, lw=.5, alpha=.8)
plt.scatter(Z_.flatten(), W_.flatten(), s=1, alpha=.1)
...
zx_crossline = map_coordinates(ZX, np.vstack((t_crossline, x_crossline))).astype(float)
zt_crossline = map_coordinates(ZT, np.vstack((t_crossline, x_crossline))).astype(float)
pz_crossline = map_coordinates(wrap(phase_Z0+phase_Z1), np.vstack((t_crossline, x_crossline))).astype(float)
pw_crossline = map_coordinates(wrap(phase_W1), np.vstack((t_crossline, x_crossline))).astype(float)

plt.figure()

plt.plot(z_crossline)
plt.plot(z1_crossline)
plt.plot(pz_crossline)
plt.plot(pw_crossline)
plt.plot(w_crossline)
...
plt.scatter(pz_crossline, wrap(pw_crossline-pz_crossline))
