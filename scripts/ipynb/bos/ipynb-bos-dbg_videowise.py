...
ax.plot(x1D, rivuletfinding.bos_videowise(frames, **kwargs)[i_frametest] * 2, color='w', ls='--', label='videowise')
ax.plot(x1D, rivuletfinding.bos_framewise(frames[i_frametest], **kwargs) * 2, color='r', ls=':', label='framewise')
ax.legend()
...
for i in range(len(frames)):
    rivuletfinding.bos_framewise(frames[i], **kwargs)
t01 = time.time()
time_frames = t01-t00
utility.log_info(f'Time (no parallel): {round(time_frames, 3)} seconds')

t10 = time.time()
rivuletfinding.bos_videowise(frames, **kwargs)
t11 = time.time()
time_video = t11-t10
...
rivuletfinding.bos_videowise(frames, **kwargs).shape
