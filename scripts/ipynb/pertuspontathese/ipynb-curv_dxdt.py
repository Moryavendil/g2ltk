...
# fig, axs = plt.subplots(1, 2, sharex='col', sharey='col', squeeze=False)
...
fig, axs = plt.subplots(1, 1, squeeze=False, figsize=utility.figsize('double', ratio=1.2), sharex=True)
imshow_kw = {'origin':'upper',
             'interpolation':'nearest',
             'aspect':'auto'}
...
ax = axs[0,0]
ax.set_title('Z (normal)')
imz = ax.imshow(Z, extent=utility.correct_extent(t, x), cmap='viridis', **imshow_kw)
ax.set_ylabel('$x$ [px]')
...
dxz = np.gradient(Z, x, axis=1)
dxt = np.gradient(Z, t, axis=0)

range_dxz = [-.6, .6]
range_dxt = [-1., 1.]
rangexy = [range_dxz, range_dxt]
extent = (*range_dxz, *range_dxt)


# <codecell>

fig, axs = plt.subplots(1, 1, squeeze=False, figsize=utility.figsize('double', ratio=1.8), sharex=True)

ax = axs[0,0]
# ax.scatter(dxz.flatten()[::83], dxt.flatten()[::83], color='k', s=5, alpha=.1)
h, xe, ye, im = ax.hist2d(dxz.flatten(), dxt.flatten(), range=rangexy, bins=[1000,1000], cmap='Purples', density=True)

# inset Axes....
xb = .1
yb = xb
x1, x2, y1, y2 = -xb, xb, -yb, yb  # subregion of the original image

W0 = .35
W, H = W0, W0 * yb/xb * range_dxz[1]/range_dxt[1]

axins = ax.inset_axes(
    [1-W-.08, 1-H-.08, W, H],
    xlim=(x1, x2), ylim=(y1, y2), xticklabels=[], yticklabels=[])
axins.imshow(h.T, extent=extent, origin="lower", cmap='Purples', aspect='auto')
axins.xaxis.tick_top()
axins.yaxis.tick_right()
axins.set_xticks(np.arange(-1, 1, .05))
axins.set_xticklabels([f'{round(v, 2)}' for v in np.arange(-1, 1, .05)])
axins.set_yticks(np.arange(-1, 1, .05))
axins.set_yticklabels([f'{round(v, 2)}' for v in np.arange(-1, 1, .05)])
# axins.tick_params(axis='y', length=0)
axins.set_xlim(-xb, xb)
axins.set_ylim(-yb, yb)

ax.indicate_inset_zoom(axins, edgecolor="k")
ax.set_xlabel(r'$\partial_t z$')
ax.set_ylabel(r'$\partial_x z$')
plt.colorbar(im, ax=ax, label='Density (normalized)')
