...
l_findmainpeak = 255 - l_
if hint is not None:
    l_findmainpeak *= utility.gaussian_unnormalized(z_, hint, (peak_width_0+peak_spacing_max)*2)
# super brutal
peak1_z = z_[np.argmax(l_findmainpeak)] 
...
ax = axes[0]
ax.plot(z, 255 - l_, color='k', ls='-', alpha=.3)
if hint is not None:
    ax.plot(z, utility.gaussian_unnormalized(z_, hint, peak_width_0+peak_spacing_0)*(255-l_.min()), color='b', ls='-', alpha=.3)
ax.plot(z, l_findmainpeak, color='k', ls='-')
# ax.plot(z, l_span, color='k', alpha=.3, label=fr'Mean $x = {x_probe} \pm {probespan}$')
# ax.plot(z_, l_, color='k', lw=2, label=fr'minus main peak (to find smaller one)')
# 
# ax.plot(z_peak2, l_peak2, color='b', lw=2, label=fr'filtered')
# 
ax.axvline(peak1_z, c='g', ls='--', alpha=.5, label='highest peak')
# ax.axvline(peak2_z, c='g', ls='--', alpha=.5)
# ax.set_xlabel('z [px]')
# ax.set_ylabel('luminosity [0-255]')
# ax.legend()
